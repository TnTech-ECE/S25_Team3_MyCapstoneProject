...
Several potential solutions will be considered for implementation. The first solution will involve using Remote ID (RID) signals via Wi-Fi/Bluetooth to track and identify the drone. The RID signal will contain data such as ID, location, altitude, and velocity. While this method will be beneficial, it will only track drones that transmit an RID signal. If a drone does not have this signal, the method will be ineffective. 
...
The third solution will utilize a spectrum analyzer and a camera to detect an RF signal, measure its power, and determine its approach angle. This approach will be the most effective for locating drones, as it will offer a larger detection range, require only one transceiver, and integrate the first method using Wi-Fi/Bluetooth to capture additional data, such as the operator's location.
