...
There were a few potential solutions this team researched. One potential solution was to use the Remote Identification (RID) signals via Bluetooth to track and identify the drone. With this, you can find the transmitted RID signal that tells you identity, location, altitude, velocity, etc. Though it would be beneficial to use this method, it would only track drones that used this RID signal. If the drone did not have this signal, this method would not work. Another solution would be to use RF signals using three transceivers to triangulate the location of the drone. With this, there would need to be multiple transceivers set up around campus. When a drone enters the area, three of the transceivers would have to pick up the signal. Hypothetically, using each of the three received signals, this team would use the distances to find the point where the signals intersect. Using this method would not do that though. It would only give you a three dimensional “column” area. The exact location of the drone could not be tracked, only a general area in which the drone would be located. Also, there would need to be multiple transceivers around campus. This could overcomplicate the system. The third solution would use a spectrum analyzer and the camera to detect RF signals and its power. This solution would be the best to locate the drone. It is a better method, because it has a larger range and can be used in weaker signal areas, like inside and around buildings. It would also only require one transceiver. This team also intends to use Wi-Fi to capture package headers in order to trace them back to the drone operator. 
...
### Timeline
<img src= "/Documents/Images/Gantt Chart for Conceptual Design.png" width="3200" height="900">
