...
### Mirco Processor (Raspberry Pi)
The mirco processor shall send a digital signal to the camera subsystem to control the camera’s movement. The mirco processor shall receive two signals from the camera subsystem, one digital signal of the photo taken of the drone and one analog to define the angle of rotation and height of the camera. The mirco processor shall receive a digital signal from the spectrum analyzer of the RF signal received from the drone. The mirco processor shall receive a power signal from the power supply subsystem. The micro processor shall receive two digital signals from the wi-fi/Bluetooth RID subsystem, one that defines the drone’s RID signal and another that is the header packets from the drone’s controller. The micro processor shall send a digital signal to the website/server/database subsystem that contains the information gathered from the camera, spectrum analyzer, and wi-fi/Bluetooth RID subsystems. The micro processor shall use machine learning to identify the drone from the photo received from the camera unit and the rf signal recived from the spectrum analyzer unit.
