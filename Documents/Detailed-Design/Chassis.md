...
Below are several images of the 3D models for both the interior and exterior shells of the chassis. The first three images model the interior shell. They show the top (lid) portion of the shell, the bottom (base) portion of the shell, and the entire shell once assembled. The design uses 16 mm M2.5 standoffs to separate the Raspberry Pi from the PoE hat, and M2.5 screws to secure the lid to the standoffs. The final image depicts the exterior shell, which features a simple, functional design. It is another lidded box, but unlike the interior shell, it does not have ventilation slits. The box has an IP67 rating, so that speicification shall be more than met. The only potential entry point for water or debris is the cable eyelet that will be drilled into the box, and that shall be protected using a 0.5 inch cable gland. A mounting plate is included with the box to attach the interior shell, and there are mounting brackets on the outside of the box to allow the chassis to be securely fastened to a lightpost using M2.5 x 16 mm screws. These brackets ensure stability and durability, even in high winds or stormy conditions. The light gray color of the box will help minimize heat absorption. 
