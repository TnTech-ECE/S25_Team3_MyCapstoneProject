...
The Raspberry Pi 5 has sufficient processing power to handle telemetry data incoming from several remote transceivers at the required sample rate. It is equipped with a 2.4 GHz quad-core ARM Cortex-A76 CPU. Each of the four cores can independently execute threads or processes, enabling the system to manage multiple simultaneous TCP connections. The A76 is built to process instructions and tasks in parallel much more effectively. This parallelism is essential for maintaining stable performance when dealing with multiple data streams. [9] This experiment tests the amount of inserts mySQL can receive. Multiple inserts were given to mySQL at a time and concluded it can handle 28,000 inserts per second. [10] This project uses real-time data that is logged into a mySQL database. Based on the experiment and project, the Raspberry Pi 5 is able to handle the data with ease.
